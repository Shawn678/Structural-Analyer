...
顯示獨立的數字輸入欄，桿件表格**保留** E/A/I/G 欄位（Phase 1 不動 UI）。快速代入區代表「全域統一截面覆蓋值」，適用於所有桿件截面相同的情況：
...
> **說明：** 此處數值為代入符號公式的實際參數，假設所有桿件使用相同截面。若各桿件截面不同，請直接執行完整分析。  
> Phase 2 預計支援截面群組（section_id），對應脊背橋鋼構 / 混凝土異質材料案例。

---

## Phase 2 預留範圍（本次不實作）

- `symbolic.py` 支援多組符號變數 `E_1/A_1/I_1/G_1, E_2/A_2/I_2/G_2`
- 桿件表格新增 `section_id` 欄位，移除直接填寫 E/A/I/G
- 截面定義表（鋼構群組 / 混凝土群組），快速代入區改為按群組輸入
- 適用於脊背橋主梁（鋼）、吊索（鋼絞線）、橋塔（混凝土）的完整異質材料分析
