...
⚡ 快速代入參數（可展開）
┌──────────────────────────────────────────────────────────────┐
│ 斷面名稱 │ E (Pa)  │ A (m²)  │ I33 (m⁴) │ G (Pa)  │ 計算器 │
│ H300    │ 200e9  │ 0.015   │ 2.4e-4   │ 77e9   │ [開啟] │  ← 可編輯
│ I200    │ 206e9  │ 0.010   │ 1.2e-4   │ 79e9   │ [開啟] │  ← 可編輯
├──────────────────────────────────────────────────────────────┤
│  （計算器展開區，同上方斷面管理的斷面性質計算器）              │
│  算完後 [填入表格] → 僅更新快速代入表格，不影響上方斷面定義   │
├──────────────────────────────────────────────────────────────┤
│  P 倍率 [1.0]             w 倍率 [0.0]                       │
│  [⚡ 代入參數（快速）]  ← 快取無效時停用                      │
└──────────────────────────────────────────────────────────────┘
...
**斷面性質計算器（嵌入式）：**  
每列斷面旁有「開啟計算器」按鈕（`st.button` 或 `st.expander`），展開後顯示與上方斷面管理區相同的計算器介面（矩形、I 型、箱型等選項 + 尺寸輸入 + Plotly 預覽）。計算完成後顯示「填入表格」按鈕，將算出的 A/I33/I22/J 等值寫入 `st.session_state["quickfill_overrides"][sec_name]`，**不修改** `st.session_state["sections"]`。計算器的 UI 邏輯直接複用上方斷面管理的現有函式，不重複實作。
...
- 在快速代入面板修改參數（包含透過嵌入式計算器填入的值）**不會**更動 `st.session_state["sections"]`，僅影響此次快速代入的計算。
